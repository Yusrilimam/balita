...
def laporan():
    ...
    # --- Chart Data ---
    # Grouping jumlah status gizi per bulan
    trend = defaultdict(lambda: Counter())
    for row in data:
        tgl = row['tanggal_ukur']
        bulan = datetime.strptime(tgl, "%Y-%m-%d").strftime("%Y-%m")
        status = row['status_gizi']
        trend[bulan][status] += 1

    labels = sorted(trend.keys())
    # Jika ingin label "Mei 2024" ganti jadi:
    # labels = [datetime.strptime(bln, "%Y-%m").strftime("%B %Y") for bln in labels]
...
def lvq_sampling():
    ...
    lvq = LVQ(n_prototypes_per_class=7, learning_rate=0.1, n_epochs=50)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                if not balita:
                    conn.execute(
                        '''INSERT INTO balita (nama, tanggal_lahir, usia_tahun, usia_bulan, jenis_kelamin, nama_ortu) 
                           VALUES (?, ?, ?, ?, ?, ?)''',
                        (row['nama'], tanggal_lahir, tahun, bulan, row['jenis kelamin'], row['nama orangtua'])
                    )
                    balita_id = conn.execute('SELECT last_insert_rowid()').fetchone()[0]
                else:
                    # Tambahkan update usia_tahun & usia_bulan jika sudah ada
                    conn.execute(
                        '''UPDATE balita SET tanggal_lahir = ?, usia_tahun = ?, usia_bulan = ? WHERE id = ?''',
                        (tanggal_lahir, tahun, bulan, balita['id'])
                    )
                    balita_id = balita['id']
